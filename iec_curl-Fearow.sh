...
function KinglerCollectLog()
{
    ...
    if [ "$FileName" == "" ];then
        res=`curl -s -X GET http://$BMC_IP/api/GetDownloadPkgName \
                     -H "X-CSRFTOKEN:$X_CSRFTOKEN" \
                     -b "$COOKIE" | awk '{print $3}'`
        res=${res#*\"}
        res=${res%\"*}
    ...
    echo "Download log ..."
    res=`curl -X GET -o $FileName http://$BMC_IP/api/DownloadOneKeyLog \
              -H "X-CSRFTOKEN:$X_CSRFTOKEN" \
              -b "$COOKIE"`
    if [ $? != 0 ];then
...
function MainCollectLog()
{
    ...
    echo "Download log ..."
    res="`curl -X GET -o $FileName http://$BMC_IP/api/ipt/download/alllogs`"
    if [ $? != 0 ];then
        DispErr "Download log ...Fail"
        return 255
    fi
...
function MainServer2Client()
{
    ...
    res="`curl -X GET -o $FileName http://$BMC_IP/api/OEMDownloadFile`"
    if [ $? != 0 ];then
...
function MainSendRawCommand()
{
    ...
    res=`curl -s -X PUT http://$BMC_IP/api/SendRawCommand \
              -d "{\"data\":\"$1\"}" \
              -H "Content-Type:application/json" \
              -H "X-CSRFTOKEN: $X_CSRFTOKEN" \
              -b "$COOKIE"`
