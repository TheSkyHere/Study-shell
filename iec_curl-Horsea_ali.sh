#!/bin/bash
#this Restful FW update script is for Horsea_ali only
...
[Options]
    ...
    -N               BMC Preserve Section Setting:
                     00h = Flash the Firmware without preserving section (default)
                     01h = Flash the Firmware with preserving section

                     BIOS Preserve Section Setting:
                     00h = Flash the Firmware without preserving boot section/conf/nvram section (default)
                     01h = Flash the Firmware with preserving boot section only
                     02h = Flash the Firmware with preserving conf/nvram section only
                     03h = Flash the Firmware with preserving both boot and conf/nvram section
...
function BMCPreserveValue()
{
    case $preserve_data in
        "" )
            preserve_string="{\"preserve_config\":0,\"flash_status\":1}"
            ;;
        0 )
            preserve_string="{\"preserve_config\":0,\"flash_status\":1}"
            ;;
        1 )
            # preserve_string="{\"preserve_config\":0,\"flash_status\":2,\"section_count\":8,\"section_names\":[\"boot\",\"conf\",\"root\",\"osimage\",\"dre\",\"www\",\"extlog\",\"horsea_a\"]}"
            preserve_string="{\"preserve_config\":1,\"flash_status\":2,\"section_count\":7,\"section_names\":[\"boot\",\"conf\",\"root\",\"osimage\",\"www\",\"extlog\",\"horsea_a\"]}"
            ;;
        * )
            DispErr "The value of BMC Preserve Section Setting is out of range"
            exit 255
            ;;
    esac

    #echo "$preserve_string"
}


function BIOSPreserveValue()
{
    case $preserve_data in
        "" )
            preserve_string="{\"preserve_config\":0,\"flash_status\":1}"
        ...
        0 )
            ...
            ;;
        1 )
            preserve_string="{\"preserve_config\":0,\"flash_status\":2,\"section_count\":1,\"section_names\":[\"FVBB\"]}"
            ;;
        2 )
            preserve_string="{\"preserve_config\":0,\"flash_status\":2,\"section_count\":1,\"section_names\":[\"NVRAM\"]}"
            ;;
        3 )
            preserve_string="{\"preserve_config\":0,\"flash_status\":2,\"section_count\":2,\"section_names\":[\"FVBB\",\"NVRAM\"]}"
            ;;
        * )
            DispErr "The value of BIOS Preserve Section Setting is out of range"
            exit 255
            ;;
    esac
...
function UploadFile()
{
    ...
    res=`curl -X POST http://$BMC_IP/api/maintenance/firmware/firmware \
              -F "fwimage=@${FileName}" \
              -H "X-CSRFTOKEN:$X_CSRFTOKEN" \
              -b "$COOKIE" | awk '{print $3}'`
    echo $res
...
while getopts "hvH:U:P:F:N:" opts
do
    case $opts in
        h )
            HelpFlag=1
...
elif [[ "$tmp" =~ "BIN" ]] || [[ "$tmp" =~ "bin" ]] || [[ "$tmp" =~ "rom" ]];then
    IMAGE_TYPE="BIOS"
    BIOSPreserveValue
    MainUpdateBIOS
    if [ $? != 0 ];then
